...
Read [Delta Sharing](https://delta.io/sharing/) tables from R as tibbles, data
frames, or Arrow objects. [Delta Kernel](https://docs.delta.io/kernel/rust/introduction.html)
handles snapshots and change data feeds, including deletion vectors and column
mapping.
...
housing_tbl <- housing$snapshot(
  columns = c("chas", "medv"),
  limit = 5
)$to_tibble()
```

For your own share, pass a profile file, discover its tables, and create a
reusable table handle:
...
`to_tibble()` is the usual eager R materializer. Readers can instead return a
base data frame with `to_data_frame()`, an Arrow table with `to_arrow()`, a lazy
Arrow reader with `to_arrow_reader()`, or an Arrow C Stream with
`to_arrow_stream()`. The Arrow table and reader methods require the optional
`arrow` package.
...
end-to-end `to_tibble()` reads at the default download concurrency of four. The
empty-cache read downloads its selected files; the cached read repeats the query
after those files have been staged. The benchmark can be rerun with
...
These measurements include the Sharing request, local log construction, Delta
Kernel scan, and tibble materialization—not just network transfer.
