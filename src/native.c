...
static void delta_sharing_collect_job_finalizer(SEXP job_xptr) {
  void *job = R_ExternalPtrAddr(job_xptr);
  if (job == NULL) {
    return;
  }
  R_ClearExternalPtr(job_xptr);
  delta_sharing_native_collect_release(job);
}

static void *delta_sharing_collect_job(SEXP job_xptr) {
  if (TYPEOF(job_xptr) != EXTPTRSXP ||
      !Rf_inherits(job_xptr, "delta_sharing_collect_job")) {
    Rf_error("`job_xptr` must be a collection job external pointer.");
  }
  void *job = R_ExternalPtrAddr(job_xptr);
  if (job == NULL) {
    Rf_error("The collection job is no longer available.");
  }
  return job;
}

static SEXP delta_sharing_collect_start(SEXP stream_xptr) {
  if (TYPEOF(stream_xptr) != EXTPTRSXP ||
      !Rf_inherits(stream_xptr, "nanoarrow_array_stream")) {
    Rf_error("`stream_xptr` must inherit from 'nanoarrow_array_stream'.");
  }
  ArrowArrayStream *stream =
      (ArrowArrayStream *)R_ExternalPtrAddr(stream_xptr);
  if (stream == NULL || stream->release == NULL) {
    Rf_error("The source Arrow stream is not initialized.");
  }

  SEXP job_xptr = PROTECT(R_MakeExternalPtr(NULL, R_NilValue, R_NilValue));
  SEXP job_class = PROTECT(Rf_mkString("delta_sharing_collect_job"));
  Rf_classgets(job_xptr, job_class);
  R_RegisterCFinalizerEx(
      job_xptr, delta_sharing_collect_job_finalizer, TRUE);

  void *job = NULL;
  char error[DELTA_SHARING_ERROR_CAPACITY] = {0};
  const int32_t status = delta_sharing_native_collect_start(
      stream, &job, error, sizeof(error));
  if (status != 0) {
    UNPROTECT(2);
    raise_native_error(status, error);
  }
  R_SetExternalPtrAddr(job_xptr, job);
  UNPROTECT(2);
  return job_xptr;
}

static SEXP delta_sharing_collect_status(SEXP job_xptr) {
  void *job = delta_sharing_collect_job(job_xptr);
  uint64_t rows = 0;
  uint64_t batches = 0;
  int32_t done = 0;
  char error[DELTA_SHARING_ERROR_CAPACITY] = {0};
  const int32_t status = delta_sharing_native_collect_status(
      job, &rows, &batches, &done, error, sizeof(error));
  if (status != 0) {
    raise_native_error(status, error);
  }
  ...
  SET_VECTOR_ELT(result, 0, Rf_ScalarReal((double)rows));
  SET_VECTOR_ELT(result, 1, Rf_ScalarReal((double)batches));
  SET_VECTOR_ELT(result, 2, Rf_ScalarLogical(done != 0));
  SET_STRING_ELT(names, 0, Rf_mkChar("rows"));
  SET_STRING_ELT(names, 1, Rf_mkChar("batches"));
  SET_STRING_ELT(names, 2, Rf_mkChar("done"));
  Rf_setAttrib(result, R_NamesSymbol, names);
  UNPROTECT(2);
  return result;
}

static SEXP delta_sharing_collect_finish(
    SEXP job_xptr, SEXP stream_xptr) {
  void *job = delta_sharing_collect_job(job_xptr);
  if (TYPEOF(stream_xptr) != EXTPTRSXP ||
      !Rf_inherits(stream_xptr, "nanoarrow_array_stream")) {
    Rf_error("`stream_xptr` must inherit from 'nanoarrow_array_stream'.");
  ...
  char error[DELTA_SHARING_ERROR_CAPACITY] = {0};
  const int32_t status = delta_sharing_native_collect_finish(
      job, stream, error, sizeof(error));
  R_ClearExternalPtr(job_xptr);
  delta_sharing_native_collect_release(job);
  if (status != 0) {
    raise_native_error(status, error);
...
static SEXP delta_sharing_collect_cancel(SEXP job_xptr) {
  if (TYPEOF(job_xptr) != EXTPTRSXP ||
      !Rf_inherits(job_xptr, "delta_sharing_collect_job")) {
    Rf_error("`job_xptr` must be a collection job external pointer.");
  }
...
static SEXP delta_sharing_collect_active(void) {
  uint64_t active = 0;
  char error[DELTA_SHARING_ERROR_CAPACITY] = {0};
  const int32_t status =
      delta_sharing_native_collect_active(&active, error, sizeof(error));
  if (status != 0) {
    raise_native_error(status, error);
  }
  return Rf_ScalarReal((double)active);
}
...
static const R_CallMethodDef call_methods[] = {
    ...
    {"delta_sharing_stream_from_cdf",
     ...
     7},
    {"delta_sharing_collect_start",
     (DL_FUNC)&delta_sharing_collect_start,
     1},
    {"delta_sharing_collect_status",
     (DL_FUNC)&delta_sharing_collect_status,
     1},
    {"delta_sharing_collect_finish",
     (DL_FUNC)&delta_sharing_collect_finish,
     2},
    {"delta_sharing_collect_cancel",
     (DL_FUNC)&delta_sharing_collect_cancel,
     1},
    {"delta_sharing_collect_active",
     (DL_FUNC)&delta_sharing_collect_active,
     0},
